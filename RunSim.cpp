...
extern "C" {
double* run_sim(double habitat_preference, double step_shape, double directional_bias, double roost_lambda,
...
//	param.push_back(5); // beta2
    ...
    double* output_arr = new double[2048];
    ...
    for(int i = 0; i < times.size(); i++) {  //todo? check last
        ...
        output_arr[i*7] = tstamp;
        output_arr[i*7+1] = x_obs;
        output_arr[i*7+2] = y_obs;
        output_arr[i*7+3] = step_obs;
        output_arr[i*7+4] = turn_angle_obs;  // TODO! do <2 thing?
        output_arr[i*7+5] = env_val_obs;
        output_arr[i*7+6] = rsc;
    }
